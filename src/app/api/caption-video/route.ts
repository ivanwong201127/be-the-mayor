...
import { Qwen2VLInput, Qwen2VLPrediction, VideoCaptioningRequest, VideoCaptioningResponse } from '@/types';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const prompt = formData.get('prompt') as string || VIDEO_CAPTIONING_CONFIG.DEFAULT_PROMPT;
    const maxNewTokens = parseInt(formData.get('maxNewTokens') as string) || VIDEO_CAPTIONING_CONFIG.DEFAULT_MAX_TOKENS;
    ...
    const isMobileVideo = videoFile.type === 'video/mp4' || videoFile.type === 'video/webm';
    
    // Chunk the video into 5-second segments
    const videoChunks = await chunkVideo(videoBlob);
    
    console.log(`Processing ${videoChunks.length} video chunks for ${isMobileVideo ? 'mobile' : 'desktop'} video`);
    ...
    for (let i = 0; i < videoChunks.length; i++) {
      ...
      try {
        ...
        console.log(`Processing video chunk ${i + 1}/${videoChunks.length} with input:`, JSON.stringify(input, null, 2));

        const response = await fetch('https://api.replicate.com/v1/models/lucataco/qwen2-vl-7b-instruct/predictions', {
          method: 'POST',
          headers: {
